---
description: LazyAgents · Store reusable guidance in the knowledge memory service.
---
...
2. **Search Before Store** — Check for existing similar entries first with `lazyagents memory search --query "<topic>"` to avoid duplicates.
3. **Validate Quality** — Ensure it is specific and reusable (not generic advice). Avoid storing secrets or sensitive data.
4. **Store** — Call `memory_storeKnowledge` with title, content, tags, scope. If MCP tools are unavailable, use `lazyagents memory store` instead.
