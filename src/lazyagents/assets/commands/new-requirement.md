---
description: LazyAgents · Scaffold feature documentation from requirements through planning.
---

Guide me through adding a new feature, from requirements documentation to implementation readiness.

1. **Use LazyAgents Memory for Context** — Search LazyAgents memory (not built-in memory) for related decisions or conventions via `lazyagents memory search --query "<feature/topic>"` and reuse relevant context. If unfamiliar, check the LazyAgents memory skill first.
...
3. **Initialize Feature Documentation Structure** — Run `lazyagents docs init-feature {name}` and fill the returned paths. It uses configured phases. If unavailable, copy each configured phase `README.md` to `docs/ai/{phase}/feature-{name}.md`, preserving frontmatter and headings.
...
7. **Store Reusable Knowledge** — When important conventions or decisions are finalized, store them via `lazyagents memory store --title "<title>" --content "<knowledge>" --tags "<tags>"`.
